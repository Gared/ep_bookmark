...
var refreshPadList = function(bookmarks) {
  ...
  if (bookmarks.length === 0) {
    ...
    var baseUrl = './';
    if ($('#bookmarkManagerIndex').length > 0) {
      baseUrl = './p/';
    }

    for (var i=0; i < bookmarks.length; i++) {
      padDiv = $("<div/>").attr('class', 'padBookmark').attr('id', bookmarks[i].padId);
      padLinkTag = $("<a class='bookmarkPadId' href='javascript:window.location.href = \"" + baseUrl + bookmarks[i].padId+"\"'>"+bookmarks[i].padId+"</a>").attr('title', html10n.get("ep_bookmark.lastVisit")+': '+new Date(bookmarks[i].timestamp).toLocaleString());
...
var registerSearchInputHandler = function() {
